...
35*     |in imm/r1,r2        |23 r2[4],mvflg[4] imm[8]/r1[4]        |端口输入         |第一个参数是端口号，第二个是储存输入的寄存器，只输入1字节
36*     |out r1,imm/r2       |24 r1[4],mvflg[4] imm[8]/r2[4]        |端口输出         |第一个是储存输入的寄存器，第二个参数是端口号，只输出寄存器的第一字节
37      |                    |25                                    |
38      |                    |26                                    |
39      |                    |27                                    |
40      |                    |28                                    |
