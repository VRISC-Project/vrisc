...
> 空白的指令编号为保留使用
>
> wide参数在指令名上体现在指令名后有b/w/d/q（字节、字、双字、四字）
...
15      |                    |12                                    |
15      |                    |13                                    |
20      |jc(w/d/q) imm       |14 16/32/64[4],cond[4] imm[16/32/64]  |跳转
21      |cc(w/d/q) imm       |15 16/32/64[4],cond[4] imm[16/32/64]  |函数调用        |将ip转储
22      |r                   |16                                    |从函数返回       |将ipdump作为返回地址
23      |loop r, imm         |17 r1[8],imm[32]                      |条件循环        |r为非零时跳转到相对地址imm处
24*     |ir imm              |18 mod[8]                             |从中断返回
25      |sysc                |19                                    |陷入内核态
26*     |sysr                |1a                                    |跳回用户态
27      |                    |1b                                    |
28      |ldi imm, r          |1c 8/16/32/64[4],r[4] imm[8/16/32/64] |加载立即数
