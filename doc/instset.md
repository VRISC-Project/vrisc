...
6       |and r1, r2, r3      |06 r1[4],r2[4] r3[4],wide[4]          |与运算
7       |or r1, r2, r3       |07 r1[4],r2[4] r3[4],wide[4]          |或运算
8       |not r1, r2          |08 r1[4],r2[4]                        |非运算
9       |xor r1, r2, r3      |09 r1[4],r2[4] r3[4],wide[4]          |异或运算
10      |jc(w/d/q) imm       |0a 16/32/64[4],cond[4] imm[16/32/64]  |指令跳转
11      |cc(w/d/q) imm       |0b 16/32/64[4],cond[4] imm[16/32/64]  |函数调用       |返回地址存于x0
12      |r                   |0c                                    |从函数返回      |跳回x0所指的地址
13*     |ir imm              |0d mod[8]                             |从中断返回
14      |sysc                |0e                                    |陷入内核态
15*     |sysr                |0f                                    |跳回用户态
16      |loop r, imm         |10 r1[8],imm[32]                      |条件循环       |r为非零时跳转到相对地址imm处
17      |shl r1, r2          |11 r1[4],r2[4] wide[8]                |左位移        |将r2左移r1位
18      |shr r1, r2          |12 r1[4],r2[4] wide[8]                |右位移
19      |rol r1, r2          |13 r1[4],r2[4] wide[8]                |左旋
20      |ror r1, r2          |14 r1[4],r2[4] wide[8]                |右旋
21      |ldi(b/w/d/q) imm, r |15 8/16/32/64[4],r[4] imm[8/16/32/64] |加载立即数
22      |ldm r1, r2          |16 r1[4],r2[4] wide[8]                |从内存加载      |将r1所指的内存处的数据加载到r2中（r1前可不加*号）
23      |stm r1, r2          |17 r1[4],r2[4] wide[8]                |保存到内存      |将r1的内容保存到r2所指的内存中（r2前可不加*号）
24*     |ei                  |18                                    |使能中断
25*     |di                  |19                                    |关闭中断
26*     |ep                  |1a                                    |使能分页
27*     |dp                  |1b                                    |关闭分页
28      |mv r1/\*m1,r2/\*m2  |1c mvflg[8] r1[4],r2[4]               |移动数据       |将r1移动至r2(不建议使用)
29*     |livt r              |1d r[8]                               |加载中断向量表
30*     |lkpt r              |1e r[8]                               |加载内核态页表
31*     |lupt r              |1f r[8]                               |加载用户态页表
32      |lipdump r           |20 r[8]                               |将ipdump加载到寄存器中
33      |lflgdump r          |21 r[8]                               |将flgdump加载到寄存器中
...
38      |sipdump r           |26 r[8]                               |将寄存器的值写入ipdump
39      |sflgdump r          |27 r[8]                               |将寄存器的值写入flgdump
40      |                    |                                      |保留使用
